...
self.onmessage = async (msg: MessageEvent<WorkerMessage>) => {
  ...
  worker.initialize((ctx: monaco.worker.IWorkerContext<WorkerHost>, { tsconfig, dependencies }: CreateData) => {
    ...
    const env: LanguageServiceEnvironment = {
      ...
      fs: createNpmFileSystem(
        ...
        (path, content) => {
          ctx.host.onFetchCdnFile(asUri("/node_modules/" + path).toString(), content)
        }
      ),
    }
...
async function importTsFromCdn(tsVersion: string) {
  ...
  const tsUrl = `https://unpkg.com/typescript@${tsVersion}/lib/typescript.js`
