import type { CodeFile, BaseModule } from "./types"
import {
  MagicString,
  babelParse,
  extractIdentifiers,
...
/**
 * 编译`BaseModule`成最终浏览器可直接使用的代码
 */
export async function compileModuleForPreview(store: BaseModule, isSSR = false) {
  const seen = new Set<CodeFile>()
  const processed: string[] = []
  const mainFile = store.files[store.main]
  if (!mainFile) {
    throw new Error(`main file not found: ${store.main}`)
  }
  processFile(store, mainFile, processed, seen, isSSR)
...
function processFile(store: BaseModule, file: CodeFile, processed: string[], seen: Set<CodeFile>, isSSR: boolean) {
  if (seen.has(file)) {
    return []
  }
  seen.add(file)
...
function processModule(store: BaseModule, src: string, filename: string) {
  ...
  function defineImport(node: Node, source: string) {
    const filename = resolveImport(source.replace(/^\.\/+/, store.dir))
    if (!filename) {
      throw new Error(`CodeFile "${source}" does not exist.`)
    }
    ...
    enter(node: Node, parent: Node) {
      if (node.type === "Import" && parent.type === "CallExpression") {
        ...
        if (arg.type === "StringLiteral" && arg.value.startsWith("./")) {
          ...
          s.overwrite(arg.start!, arg.end!, JSON.stringify(arg.value.replace(/^\.\/+/, store.dir)))
        }
      }
    },
  })
...
function processHtmlFile(store: BaseModule, src: string, filename: string, processed: string[], seen: Set<CodeFile>) {
  const deps: string[] = []
  let jsCode = ""
  const html = src
